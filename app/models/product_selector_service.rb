class ProductSelectorService
  ...
  def self.product_selector(string)
    string.gsub!(@@STOPWORDS,'').downcase!
    input_array = string.split(" ")
    if input_array.include?("disposable")
      return score(input_array, @@DISPOSABLES).sort_by { |_, v| -v }[0..1].map(&:first)
    elsif input_array.include?("refill")
      return score(input_array, @@REFILLS).sort_by { |_, v| -v }[0..1].map(&:first)
    else
      return score(input_array, @@UNITS).sort_by { |_, v| -v }[0..1].map(&:first)
    end
  end
end